...
if __name__ == "__main__":
    ...
    pc_df = pd.read_csv(f'data\\tma_both_cleaned_pca.csv')
    meta_df = pd.read_csv(f'data\\tma_both_cleaned_meta.csv')
    ...
    def run_harmony(
        ...
        h0 = harm_Ours(
            data_mat, phi, phi_moe, Pr_b, sigma, theta, 0, max_iter_kmeans,
            epsilon_cluster, epsilon_harmony, nclust, block_size, lamb_mat, verbose,
            random_state, cluster_fn
        )
    ...
    hh,ho,h0 = run_harmony(pc_df, meta_df, ['donor'])

    import umap.umap_ as umap
    import pandas as pd

    hh_reducer = umap.UMAP(n_neighbors=15, min_dist=0.1, n_components=2, random_state=42)
    hh_umap_results = hh_reducer.fit_transform(hh.Z_corr.T)

    ho_reducer = umap.UMAP(n_neighbors=15, min_dist=0.1, n_components=2, random_state=42)
    ho_umap_results = ho_reducer.fit_transform(ho.Z_corr.T)

    h0_reducer = umap.UMAP(n_neighbors=15, min_dist=0.1, n_components=2, random_state=42)
    h0_umap_results = ho_reducer.fit_transform(h0.Z_corr.T)

    # Create a DataFrame of the results of each method
    hh_df = pd.DataFrame(hh.Z_corr.T, columns=[f"PC {i}" for i in range(1, 31)])
    hh_df.to_csv("harmony.csv",  index=False)

    ho_df = pd.DataFrame(ho.Z_corr.T, columns=[f"PC {i}" for i in range(1, 31)])
    ho_df.to_csv("ours.csv",  index=False)

    h0_df = pd.DataFrame(h0.Z_corr.T, columns=[f"PC {i}" for i in range(1, 31)])
    h0_df.to_csv("origin.csv",  index=False)

    # Create a DataFrame of the results of each method
    hh_df_umap = pd.DataFrame(hh_umap_results, columns=['UMAP 1', 'UMAP 2'])
    ho_df_umap = pd.DataFrame(ho_umap_results, columns=['UMAP 1', 'UMAP 2'])
    h0_df_umap = pd.DataFrame(h0_umap_results, columns=['UMAP 1', 'UMAP 2'])

    hh_df_umap.to_pickle('harmony.pkl')
    ho_df_umap.to_pickle('ours.pkl')
    h0_df_umap.to_pickle('origin.pkl')
