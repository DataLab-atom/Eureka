...
if __name__ == "__main__":
    ...
    meta_data = pd.read_csv("dataset/pbmc_3500_meta.tsv.gz", sep="\t")
    data_mat = pd.read_csv("dataset/pbmc_3500_pcs.tsv.gz", sep="\t")
    ...
    ho,hh = run_harmony(data_mat, meta_data, ['donor'])
    

    lisi_ho = compute_lisi(ho.result().T, meta_data, ['donor'])
    lisi_hh = compute_lisi(hh.result().T, meta_data, ['donor'])

    res_ho = np.mean(lisi_ho)
    res_hh = np.mean(lisi_hh)

    summary = pd.DataFrame({
        "Method": ["harm_Ours", "Harmony_original"],
        "Mean_LISI_donor": [res_ho, res_hh]
    })
    summary.to_csv("./experimental_result_data/lisi_summary.csv", index=False)